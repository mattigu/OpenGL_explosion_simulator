...
bool Program::Load(std::string vertexShaderFilePath, std::string fragmentShaderFilePath, std::string geometryShaderFilePath)
{
    ...
    status = readFile(fragmentShaderFilePath, fragmentShaderSource);

    if (!status)
    {
        std::cerr << "Error loading fragment shader file " << fragmentShaderFilePath << std::endl;
        return false;
    }
...
bool Program::linkProgram(const GLuint vertexShaderID, const GLuint fragmentShaderID, const GLuint geometryShaderID)
{
    GLuint program;
